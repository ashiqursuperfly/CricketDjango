...
def check_if_match_valid(match_id, match_data):
    state = match_data.get("state")
    if state == "mom" and match_id not in invalid_matches:
        return True
    else:
        return False
...
def init_players_to_csv(data_dict, file_name):
    ...
    for match_id, match in data_dict.get('matches').items():
        if not check_if_match_valid(match_id, match):
            continue
...
def init_teams_to_csv(data_dict, file_name):
    ...
    for key, match in data_dict.get('matches').items():
        if not check_if_match_valid(key, match):
            continue
        # print(key)
...
def init_series_to_csv(data_dict, file_name):
    ...
    for match_key, match_value in data_dict.get('matches').items():
        if not check_if_match_valid(match_key, match_value):
            continue
        series_value = match_value.get('series')
        series_writer.writerow([series_value.get('id'), series_value.get('name'), '', ''])
...
def init_match_to_csv(data_dict, file_name):
    ...
    for match_key, match_value in data_dict.get('matches').items():
        ...
        if match_id in invalid_matches:
            continue;
...
def init_match_performance_data(match_id, team1_id, team2_id):
    ...
    path_name = "match_performance_data/"
    create_directory_if_not_exist(path_name)
...
def init_match_performance_data_wtests(match_id, team1_id, team2_id, match_type):
    ...
    team1_inn1_dictionary = create_players_dictionary(team1_inn1_batting_data)
    team2_inn2_dictionary = create_players_dictionary(team2_inn2_batting_data)
    ...
    if match_type == 'TEST':
        try:
            ...
            team1_inn3_dictionary = create_players_dictionary(team1_inn3_batting_data)
            team2_inn4_dictionary = create_players_dictionary(team2_inn4_batting_data)
            ...
            invalid_matches.insert(0, match_id)
            return -1, -1, -1, -1
    # print(team1_batting_data_array[0].prettify())
    ...
    for playerdata in team1_inn1_batting_data_array:
        ...
        playerdatacontents = playerdata.contents
        temp_model = create_player_performance_model(playerdatacontents[1].a.get('href'))
        player_model = team1_inn1_dictionary.get(temp_model.player_id)
    ...
    for playerdata in team2_inn2_batting_data_array:
        ...
        playerdatacontents = playerdata.contents
        temp_model = create_player_performance_model(playerdatacontents[1].a.get('href'))
        player_model = team2_inn2_dictionary.get(temp_model.player_id)
    ...
    if match_type == 'TEST':
        for playerdata in team1_inn3_batting_data_array:
            ...
            playerdatacontents = playerdata.contents
            temp_model = create_player_performance_model(playerdatacontents[1].a.get('href'))
            player_model = team1_inn3_dictionary.get(temp_model.player_id)
        ...
        for playerdata in team2_inn4_batting_data_array:
            ...
            playerdatacontents = playerdata.contents
            temp_model = create_player_performance_model(playerdatacontents[1].a.get('href'))
            player_model = team2_inn4_dictionary.get(temp_model.player_id)
    ...
    for player_model in team1_inn1_dictionary.values():
        match_performance_writer.writerow(
    ...
    for player_model in team2_inn2_dictionary.values():
        match_performance_writer.writerow(
    ...
    if match_type == 'TEST':
        for player_model in team1_inn3_dictionary.values():
            match_performance_writer.writerow(
        ...
        for player_model in team2_inn4_dictionary.values():
            match_performance_writer.writerow(
